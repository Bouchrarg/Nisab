"""
text_cleaning.py — Nettoyage des artefacts d'extraction PDF (numéros de notes
de bas de page collés aux mots, ruptures de page avec en-tête répété, blocs
de définitions de notes insérés en plein milieu d'un article).

Appliqué ICI à l'extraction (extract_corpus.py), donc tout nouveau document
ingéré (CGI, Bulletin Officiel, futures notes DGI) part propre dans
corpus.db — pas besoin de rejouer un nettoyage a posteriori.

Copie volontaire de backend/app/text_cleaning.py : corpus-pipeline est un
outil autonome (son propre requirements.txt, pas de dépendance vers
backend/), donc pas d'import cross-repo. Si la logique de nettoyage évolue,
