...
import { Plus, ArrowRight, Building2, Bell, Scale, CalendarClock, PiggyBank } from 'lucide-react'
...
export default function CabinetOverviewPage({ onOpenDossier }) {
  ...
  const feuFor = (dossierId) => {
    ...
    if ((s.risks?.rouge || 0) > 0) return { cls: 'critique', label: `${s.risks.rouge} anomalie(s) critique(s)` }
    if ((s.risks?.orange || 0) > 0) return { cls: 'vigilance', label: `${s.risks.orange} alerte(s)` }
    return { cls: 'conforme', label: 'Conforme' }
  }

  const criticalAlerts = dossiers
    ...
    <div>
      <div className="section-header">
        <div>
          ...
          <div className="section-sub">Vue globale du cabinet — cliquez sur un dossier pour voir son détail.</div>
        </div>
      ...
      {dossiers.length > 0 && (
        <div className="kpi-grid">
          ...
          <div className="kpi-card">
            ...
            <div className="kpi-sub">
              {portfolioStats.nbNonAnalyses > 0
                ? `sur ${analysedSummaries.length} dossier(s) analysé(s)`
          ...
          <div className="kpi-card">
            ...
            <div className="kpi-sub">
              {portfolioStats.nbNonAnalyses > 0
                ? `${portfolioStats.nbNonAnalyses} dossier(s) non analysé(s), hors total`
          ...
          <div className="kpi-card">
            ...
            <div className={`kpi-value ${portfolioStats.nbEnAlerte > 0 ? 'vigilance' : 'conforme'}`}>
              {portfolioStats.nbEnAlerte} / {analysedSummaries.length}
            </div>
            <div className="kpi-sub">au moins une anomalie critique</div>
          </div>
        </div>
      )}
      ...
      {/* Valeur générée : deux chiffres MESURÉS (exposition détectée /
          ...
          (temps épargné), dont l'hypothèse est affichée à côté plutôt que
          cachée derrière le nombre. Un ROI dont l'hypothèse est cachée est
          exactement le genre d'affirmation non sourcée que le reste du
          produit s'interdit. */}
      {roi && roi.status === 'ok' && roi.nb_dossiers > 0 && (
        // margin-bottom 20 pour reprendre le même rythme vertical que
        // .kpi-grid juste au-dessus (margin-bottom: 20, App.css) — sans lui,
        // la carte collait directement à .dossier-cards-grid, qui n'a
        // elle-même aucune marge haute.
        <div className="card" style={{ marginTop: 16, marginBottom: 20 }}>
          <div className="card-header">
            ...
            <span style={{ fontSize: 11, color: 'var(--sourdine)' }}>
              {roi.nb_dossiers} dossier(s) analysé(s)
              {roi.nb_dossiers_non_analyses > 0 ? ` · ${roi.nb_dossiers_non_analyses} non analysé(s), hors calcul` : ''}
            </span>
          </div>
          <div className="card-body" style={{ display: 'flex', gap: 28, flexWrap: 'wrap' }}>
            <div>
              <div style={{ fontSize: 11, color: 'var(--sourdine)', marginBottom: 2 }}>Exposition détectée avant contrôle</div>
              <div className="mono" style={{ fontSize: 20, fontWeight: 700, color: 'var(--critique)' }}>
                {roi.exposition_detectee_dh.toLocaleString('fr-MA')} DH
              </div>
              <div style={{ fontSize: 10.5, color: 'var(--sourdine)' }}>mesuré, dossiers analysés uniquement</div>
            </div>
            <div>
              <div style={{ fontSize: 11, color: 'var(--sourdine)', marginBottom: 2 }}>Exposition régularisée</div>
              <div className="mono" style={{ fontSize: 20, fontWeight: 700, color: 'var(--conforme)' }}>
                {roi.exposition_regularisee_dh.toLocaleString('fr-MA')} DH
              </div>
              <div style={{ fontSize: 10.5, color: 'var(--sourdine)' }}>mesuré, anomalies marquées « traitée »</div>
            </div>
            <div>
              <div style={{ fontSize: 11, color: 'var(--sourdine)', marginBottom: 2 }}>Temps de revue estimé épargné</div>
              <div className="mono" style={{ fontSize: 20, fontWeight: 700, color: 'var(--ardoise)' }}>
            ...
            {/* Distinct de "Exposition détectée" : celle-ci vient d'anomalies
                ...
                (`nb_echeances_base_connue` le dit explicitement). */}
            {roi.nb_echeances_suivies > 0 && (
              <div>
                <div style={{ fontSize: 11, color: 'var(--sourdine)', marginBottom: 2 }}>Exposition échéances suivies</div>
                <div className="mono" style={{ fontSize: 20, fontWeight: 700, color: 'var(--vigilance)' }}>
            ...
            return (
              <div key={d.id} className="dossier-card" onClick={() => onOpenDossier(d)}>
                <div className="dossier-card-top">
                  ...
                  <Badge cls={feu.cls} dot>{feu.label}</Badge>
                </div>
                {d.secteur_activite && <div className="dossier-card-sector">{d.secteur_activite}</div>}
      ...
      <div className="overview-widgets-grid" style={{ marginTop: 20 }}>
        <div className="card">
          <div className="card-header">
            <span className="card-title" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <div className="card-body" style={{ padding: 0, height: 420, overflowY: 'auto' }}>
            {dossiers.length === 0 ? (
              ...
              upcomingEcheances.map((e, i) => {
                ...
                return (
                  <div
                    key={`${e.dossierName}-${e.date}-${i}`}
                    style={{
                      display: 'flex', alignItems: 'center', gap: 12, padding: '10px 16px',
                      borderBottom: i < upcomingEcheances.length - 1 ? '1px solid var(--bordure)' : 'none',
                    }}
                  >
                    {dt && (
                      <div style={{ textAlign: 'center', minWidth: 34 }}>
                        <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--encre)', lineHeight: 1 }}>{dt.day}</div>
                        <div style={{ fontSize: 10, color: 'var(--sourdine)', textTransform: 'uppercase' }}>{dt.month}</div>
                      </div>
                    )}
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 12.5, fontWeight: 600, color: 'var(--encre)', lineHeight: 1.3 }}>{e.title}</div>
                      <div style={{ fontSize: 11, color: 'var(--sourdine)' }}>{e.dossierName}</div>
                    </div>
                    <Badge cls={URGENCY_CLS[e.urgency] || 'neutral'}>{e.category}</Badge>
                  </div>
                )
              })
            )}
              ...
              lawFeed.map((item, i) => (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                    <Badge cls={LAW_TYPE_CLS[item.type] || 'neutral'}>{item.type}</Badge>
                  ...
                  <div
                    style={{
                      fontSize: 11.5, color: 'var(--sourdine)', lineHeight: 1.5,
